...
public class Student {
    ...
    public async Task<StudentInfoJson> Info() {
        if (_info == null) _info = 
            await getObject<StudentInfoJson>(APIs.getLoginInfoStudentAPIEndpoint(_student), DemoJsons.CARLETTUCCINO_INFO_JSON);
        return _info;
    }
    public async Task<ObservableCollection<SubjectJson>> Subjects() {
        if (_subject != null) _subject = 
            new ObservableCollection<SubjectJson>(await getObject<List<SubjectJson>>(APIs.getSubjectsAPIEndpoint(_student), DemoJsons.CARLETTUCCINO_SUBJECTS_JSON));
        return _subject;
    }
    ...
    public async Task<ObservableCollection<ReportCardJson>> ReportCards() {
        if (_reportCards == null) _reportCards = 
            new ObservableCollection<ReportCardJson>(await getObject<List<ReportCardJson>>(APIs.getReportCardAPIEndpoint(_student), DemoJsons.CARLETTUCCINO_REPORT_CARD_JSON));
        return _reportCards;
    }
    public async Task<ObservableCollection<ComunicationThreadJson>> ComunicationThread() {
        if (_comunicationThread == null) _comunicationThread =
            new ObservableCollection<ComunicationThreadJson>(await getObject<List<ComunicationThreadJson>>(APIs.getComunicationThreadsAPIEndpoint(_student), DemoJsons.CARLETTUCCINO_COMUNICATION_THREAD_JSON));
    ...
    public async Task<ObservableCollection<ComunicationUserJson>> ComunicationUser() {
        if (_comunicationUser == null) _comunicationUser =
            new ObservableCollection<ComunicationUserJson>(await getObject<List<ComunicationUserJson>>(APIs.getComunicationsUserAPIEndpoint(_student), DemoJsons.CARLETTUCCINO_COMUNICATION_USER_JSON));
